...
lldb::offset_t DataExtractor::CopyData(lldb::offset_t offset, lldb::offset_t length,
                                       void *dst) const {
  const uint8_t *src = PeekData(offset, length);
  if (src) {
    ::memcpy(dst, src, length);
...
lldb::offset_t
DataExtractor::CopyByteOrderedData(lldb::offset_t src_offset, lldb::offset_t src_len,
...
// will not be updated.
const char *DataExtractor::GetCStr(lldb::offset_t *offset_ptr, lldb::offset_t len) const {
